...
export function Packaging() {
  const details = [
    {
      icon: Package,
      text: 'Polipropileno fosco',
    },
    {
      icon: Shield,
      text: 'Proteção contra fotodegradação',
    },
    {
      icon: Stamp,
      text: 'Formato retrátil higiênico',
    },
    {
      icon: FileText,
      text: 'Rotulagem INCI',
    ...
    {
      icon: FileText,
      text: 'RDC nº 752/2022',
    },
    {
      icon: Shield,
      text: 'Grau de Risco 2 (RDC nº 7/2015)',
    },
  ];
          ...
          <div className="fade-in-section opacity-0 translate-y-8 transition-all duration-1000">
            <div className="grid grid-cols-2 gap-6">
              {[1, 2, 3, 4].map((item) => (
                ...
                >
                  <Package style={{ color: 'var(--forest-medium)', width: '3rem', height: '3rem' }} />
                </div>
              ))}
            </div>
          </div>
              ...
              {details.map((detail, index) => (
                <div key={index} className="flex items-start gap-4">
                  <detail.icon
                    ...
                    style={{ color: 'var(--gold)', width: '1.5rem', height: '1.5rem' }}
                  />
                  <p
                    style={{
                      color: '#1A2E1F',
